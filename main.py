...
if "output" in st.session_state:
    ...
    goal_text = st.session_state.auto_goal
    ...
    if st.session_state.get("hypothesis_confirmed") and st.session_state.selected_index is not None:
        ...
        expected_lift_val = criteria.get("expected_lift", "N/A")
        try:
            expected_lift_float = float(expected_lift_val)
            expected_lift_str = f"{expected_lift_float}{unit}"
        except:
            expected_lift_str = expected_lift_val

        try:
            mde_val = float(criteria.get("MDE", 0))
            mde_display = f"{round(mde_val * 100)}%"
        except:
            mde_display = "N/A"
